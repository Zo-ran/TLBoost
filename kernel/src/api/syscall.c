...
static void handleRegister(void)
{
    ...
    portInfo = getRegister(thread,  R4);
    funcInfo = getRegister(thread,  R5);
#endif
    ...
    // printf("register thread: %lu, Received port number:%lu\n",(unsigned long)thread, (unsigned long)portInfo);
    // printf("Received func number:%lu\n",(unsigned long)funcInfo);
    // printf("Received stk number:%lu\n",(unsigned long)portInfo);
    // printf("Received info number:%lu\n",(unsigned long)info.words[0]);//(long unsigned int)mr0 (unsigned long)info.words[0]
    newIpc.ServerThread=thread;
    newIpc.func=(void *)funcInfo;
    newIpc.stk=(void *)portInfo;
    // ipcServerList[(int)portInfo]=newIpc;
...
#if defined(CONFIG_ARCH_AARCH32) || defined(CONFIG_ARCH_AARCH64)
    setRegister(thread, R4,(seL4_Word)ipcServerList_top);   
#endif
#ifdef CONFIG_ARCH_RISCV
