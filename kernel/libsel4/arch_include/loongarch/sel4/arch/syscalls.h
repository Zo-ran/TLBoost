...
LIBSEL4_INLINE_FUNC seL4_MessageInfo_t seL4_IpcRegister(seL4_CPtr dest, seL4_MessageInfo_t msgInfo,seL4_Word port,seL4_Word func)
{
    ...
    loongarch_sys_send_recv(seL4_SysRegister, dest, &dest, msgInfo.words[0], &info.words[0], &msg0, &msg1,
        &msg2, &msg3, 0);
    /* Write out the data back to memory. */
    seL4_SetMR(0, msg0);
    seL4_SetMR(1, msg1);
...
LIBSEL4_INLINE_FUNC seL4_Word* seL4_GetMSG(seL4_Word *msg)
{
    asm volatile(
        "move    $a6, %[msg0]\n"
        "st.d $a2, $a6, 0\n"   // 将 r2 中的值存储到 msg[0]
        ...
        : [msg0] "=r" (msg) // 输出约束
        :                     // 输入约束（这里没有输入）
        : "a2", "a3", "a4", "a5","a6" // 告诉编译器我们使用了这些寄存器
    );
    return msg;
}
LIBSEL4_INLINE_FUNC seL4_MessageInfo_t seL4_CallBoost(seL4_CPtr dest, seL4_MessageInfo_t msgInfo,seL4_Word port)
{
    ...
    loongarch_sys_send_recv(seL4_SysCallBoost, dest, &dest, msgInfo.words[0], &info.words[0], &msg0, &msg1,
        &msg2, &msg3, 0);
